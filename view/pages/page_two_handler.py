...
class PageTwoHandler(QObject):
    initial_rules_ready = Signal(object)
    optimized_rules_ready = Signal(object)
    ...
    def start_mining(self):
        ...
        if not self._parent.dataset_config_info or not self._parent.dataset_config_info.get('dataset_config'):
            self._parent.on_common_error("请先在页面一配置数据集")
            return
        ...
        data_frame = cleaned_df if cleaned_df is not None else raw_df
    ...
    def on_parameter_changed(self, *args, **kwargs):
        """参数调整回调占位，方便未来扩展"""
        pass

    def _handle_log_message(self, message: str):
        if self._skip_cleaning_logs:
            skip_keywords = ("数据质量", "数据清洗", "异常值", "字符串数据", "清洗报告")
